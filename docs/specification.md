...
**Default-deny SSRF policy.** RUN-step egress refuses connections to host-internal destinations by default, checked on the destination address (at connect time for the rootless userspace stack; as a `forward`-hook drop set for the rootful veth path and for a bootable build's micro-VM tap): loopback (and the unspecified address), link-local (including the `169.254.169.254` cloud-metadata IP), RFC1918, IPv6 unique-local, and CGNAT shared space. This is enforcement, not advice, and mirrors the block-all posture of [EXPOSE](#expose): a build's RUN steps cannot reach the host's own services, the cloud-metadata endpoint, or the local network unless an operator opts in. Re-allow categories with `--rootless-net-allow` (or `UMF_ROOTLESS_NET_ALLOW`), for instance to reach an internal package mirror:
...
**Bootable builds obey the same set.** A bootable build's `RUN` steps execute in a micro-VM, which attaches a tap device in a network namespace whose `forward` hook carries the identical default-deny CIDR set. The VMM's own user-mode network stack is deliberately *not* used for this: it is the VMM's, not UMF's, so nothing could be enforced on it.

Building that namespace requires `CAP_NET_ADMIN`, which therefore becomes a requirement for a bootable build's `RUN` steps. A build that cannot create it **fails** rather than falling back to the unpoliced stack — a silent fallback would make this guarantee depend on how the build happened to be launched, which is the same as not having it.

The backends, the address categories, and the operator workflow are detailed in the reference-implementation docs ([CLI](cli.md), [Prerequisites](prerequisites.md), [Troubleshooting](troubleshooting.md)). The normative point is the security posture: outbound egress from a RUN step is namespaced, and host-internal destinations are denied by default — in both build shapes.
