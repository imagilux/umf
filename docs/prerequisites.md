...
| **`CAP_NET_ADMIN`** + **`nft`** | A bootable build's `RUN` steps egress through a tap in a network namespace whose `forward` hook enforces the [default-deny SSRF policy](specification.md#run-step-network-egress) — the same set container builds obey. Creating that namespace needs the capability; programming the ruleset needs `nft`. There is **no unpoliced fallback**: a build that cannot create it fails rather than quietly dropping to the VMM's own unenforceable network stack. Only needed when the bootable build has `RUN` steps. | run as root, or `sudo setcap cap_net_admin+ep $(command -v umf)`; `sudo apt-get install -y nftables` | same, `sudo dnf install -y nftables` |
| **`ukify`** (from `systemd-ukify`, plus `systemd-boot-efi` for the EFI stub) | **Required** by `umf compile` when the image carries `LABEL org.imagilux.umf.flavor=uki`: the Unified Kernel Image is assembled by shelling out to `ukify`, with no in-process fallback, so a compile without it fails. Not needed for the default `systemd-boot` (classic) flavor, which reads its bootloader from the image rootfs. Because `ukify` wraps the kernel in the **host's** EFI stub, a UKI can only be built for the host architecture — see [Known limitations](known-limitations.md#cross-architecture-uki-projection). | `sudo apt-get install -y systemd-ukify systemd-boot-efi` | `sudo dnf install -y systemd-ukify systemd-boot` |
...
The **Cloud Hypervisor** backend (`--vmm=ch`) is an alternative to QEMU. Disk boot always needs a firmware payload (CH has no built-in fallback): `umf run` discovers a dedicated CloudHv edk2 build first — `CLOUDHV.fd` / `CLOUDHV_EFI.fd` at `/usr/share/cloud-hypervisor/` or the `edk2` package trees — then the OVMF/AAVMF list above, and `--firmware PATH` overrides. Prefer installing the CloudHv build (CH's device model is built for it; `umf doctor`'s *CH UEFI firmware* row shows what discovery would pick). Its `-p/--port-forward` is wired host-side (a per-VM netns + tap + nft DNAT, pure-Rust with no `iproute2`), so it additionally needs **`nft`** on `PATH`, plus **`dnsmasq`** for the default in-namespace DHCP (not required if you pass `--dhcp-command` to run your own DHCP daemon, or `--dhcp-command none`); `umf doctor` reports them. See [Known limitations](known-limitations.md).
...
| Build/run **bootable** images | QEMU, KVM group membership, OVMF (plus `ukify` for `flavor=uki`) |
| Bootable builds with **`RUN` steps** | the above, plus `CAP_NET_ADMIN` + `nftables` for policed RUN egress |
