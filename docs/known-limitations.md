...
- **Port-forwarding is host-side and needs the guest to take a DHCP lease.** Unlike QEMU's user-mode `hostfwd`, `-p/--port-forward --vmm=ch` sets up a per-VM network namespace + tap + nft DNAT, with a detached `dnsmasq` leasing the guest its address (gateway via the host veth). The guest must run a DHCP client, or you run your own DHCP in that namespace; `umf doctor` reports whether `dnsmasq` is present, and `nft` is required. `ip` is **not** — the addresses, routes and veth pair are configured in-process through `rtnetlink`, so `iproute2` need not be installed.
