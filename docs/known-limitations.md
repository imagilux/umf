...
- **Single-id mapping.** The map is container root to your user only. Base-image files owned by a non-root uid, and `RUN --user <nonzero>`, resolve to `nobody`. Faithful multi-uid ownership (idmapped lower layers or a subordinate range) is not yet implemented for the one-namespace path; build as root when it matters.
