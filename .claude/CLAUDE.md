...
3. **Sovereignty-first** — a *build* never requires a registry. Every component is itself an artifact produced by an ordinary `umf build` from its own recipe, and a build's output lands in the local cache, so an air-gapped operator builds components in dependency order with no network at all. A registry is where finished artifacts are published and retrieved — an accelerator and a distribution mechanism, never a precondition. UMF does **not** fetch or build a component's sources on the author's behalf: where those live is the author's concern, and an unresolvable reference is an error, not a trigger.
4. **Composable supply chain** — components are independently versioned; the same `local cache → registry` resolution applies to every OCI reference: `FROM` and every `ADD <oci-ref>` (the userland base, formerly the `ROOTFS` directive). Cache first, so a pre-warmed node never reaches the network; a reference that resolves to neither fails with an error rather than being built implicitly.
...
- **Project**: `compile` — projects a `type=bootable` OCI image to a VM or bare-metal disk image (GPT/ESP, UEFI; classic systemd-boot or UKI), reading the `org.imagilux.umf.flavor` label plus the image's layers. `--fs squashfs|ext4|erofs` picks the root filesystem (default squashfs, written in-process; the other two need the host `mkfs`) — a projection choice, never a recipe one. This is the former bootable `--disk-out` path, now a separate step.
...
│   │   ├── Cargo.toml
│   │   └── src/
│   └── umf-compile/             # disk projection — type=bootable OCI image → GPT/ESP/UEFI disk (squashfs/ext4/erofs root, systemd-boot or UKI); powers umf compile
│       ├── Cargo.toml
│       └── src/
...
- `umf-builder` — AST → OCI image. FROM resolution + introspection (the container-vs-bootable decision), container-target lowering through `umf-engine`, micro-VM RUN execution through `umf-vmm` (bootable build), boot-manifest labels, EXPOSE lowering (OCI `config.ExposedPorts` on a container, default-deny nftables on an init-system bootable image), secrets handling. Emits the OCI image (container or `type=bootable`); disk projection lives in `umf-compile`. Depends on `umf-core` + `umf-oci` + `umf-engine` + `umf-vmm` + `umf-networking` (directly — the `ADD <url>` fetch path enforces the SSRF policy itself); deliberately does **not** depend on `umf-parser` outside dev-dependencies.
- `umf-compile` — disk projection. Takes a `type=bootable` OCI image and emits a bootable disk: GPT partition table + ESP/FAT32 (`gpt`, `fatfs`), the root filesystem chosen by `umf compile --fs` (squashfs in-process via `backhand`; ext4 / erofs via the host `mkfs.ext4` / `mkfs.erofs`, no fallback), and the boot chain (UKI or systemd-boot per the `flavor` label), with symlink-contained reads of in-image boot files. Powers `umf compile`. Depends on `umf-core` + `umf-oci`.
- `umf` (binary, in `src/`) — CLI wiring. Depends on all eight library crates directly: `umf-core`, `umf-parser`, `umf-oci`, `umf-networking`, `umf-engine`, `umf-vmm`, `umf-builder`, `umf-compile`.
