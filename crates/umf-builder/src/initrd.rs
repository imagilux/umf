...
///
/// `modules.dep` is generated by `depmod` beside the module tree and lists,
/// per module, the modules it needs — paths relative to the modules root.
/// A tree without one (or with an unreadable one) keeps the selected set
/// as-is: fewer modules is the pre-existing behaviour, and the init script
...
fn with_dependencies(selected: Vec<PathBuf>, modules_root: &Path) -> Vec<PathBuf> {
    ...
    while let Some(module) = queue.pop() {
        if !resolved.insert(module.clone()) {
            continue; // already walked — also what terminates a dependency cycle
        }
        let Ok(relative) = module.strip_prefix(modules_root) else {
            continue;
        };
        let Some(needed) = deps.get(relative) else {
            continue;
        };
        for dep in needed {
            let path = modules_root.join(dep);
            // A dependency the tree does not actually carry is skipped
            // rather than fatal: it is compiled in, and nothing to embed.
            if path.is_file() {
                queue.push(path);
            }
        }
...
/// Parse `modules.dep` into `module -> direct dependencies`, every path
/// relative to the modules root exactly as the file spells it.
///
/// Each line is `<module>: <dep> <dep> …`; `depmod` already writes the full
/// transitive set per line, but this does not rely on that — it walks the
/// graph itself, so a hand-written or partial file still resolves.
fn parse_modules_dep(modules_root: &Path) -> BTreeMap<PathBuf, Vec<PathBuf>> {
