...
impl LayerResolve for FromKernelResolver {
    ...
    fn post_pull_check(layout: &ImageLayout, ref_name: &str) -> Result<(), Self::Error> {
        ...
        // A `type=bootable` image is a valid FROM: the spec says extending one
        // keeps it bootable, and `is_valid_from` has always agreed. Its layers
        // already carry the merged userland + kernel tree, so the caller skips
        // nothing — it unpacks them exactly as it would a kernel artifact's —
        // and inherits the base's boot manifest for anything the extending
        // recipe leaves unsaid.
        ...
        if profile.kind != L0Kind::Payload(Payload::Kernel) {
            return Err(FromKernelResolveError::MalformedArtifact {
                ...
                detail: format!(
                    "expected org.imagilux.umf.type=kernel or =bootable, got {:?}",
                    profile.kind
                ),
            });
        }
